...
The checked-in contract previously described the middleware as Authorization-header-only and stated that normal login/register did not set `gapak_at`; that description was stale relative to the supplied backend repository. The actual auth controller sets `gapak_at` on register, anonymous register, login, refresh, and OAuth callback.
