...
test('authoritative auth routes are represented without invented endpoints', () => {
  ...
  for (const route of ['/auth/csrf', '/auth/register', '/auth/register-anonymous', '/auth/login', '/auth/logout', '/auth/2fa/setup', '/auth/2fa/verify', '/auth/2fa/disable']) {
    assert.match(s, new RegExp(route.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
  }
  assert.match(s, /\/auth\/logout/);
  assert.match(read('src/shared/api/httpClient.ts'), /\/auth\/refresh/);
