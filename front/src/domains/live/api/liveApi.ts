...
import type { LiveChatMessage, LiveEvent, LiveStream } from '../../../shared/api/backendContracts';
...
export const liveApi = {
  ...
  start: (streamId: string, idempotencyKey: string) => httpClient.post<LiveStream>(`/live-streams/${encodeURIComponent(streamId)}/start`, undefined, { idempotencyKey }),
  end: (streamId: string, idempotencyKey: string) => httpClient.post<LiveStream>(`/live-streams/${encodeURIComponent(streamId)}/end`, undefined, { idempotencyKey }),
  join: (streamId: string, payload: { role: string; isGhostMode: boolean }, idempotencyKey: string) => httpClient.post<{ accepted: boolean }>(`/live-streams/${encodeURIComponent(streamId)}/join`, payload, { idempotencyKey }),
